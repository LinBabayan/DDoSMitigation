...
class AggregationByProtocol(object):
   def aggregateParsedData(self, files, output_file_path):
       try:
           ...
           agg_df = df.groupby(['second', 'protocol']).agg(
                pps=('length', 'count'),
                gbps=('bits', 'sum')
            ).reset_index()
...
if __name__ == "__main__":
    ...
    output_path = f"{options.train_data_folder}Agg\\AggregateByProtocol\\Monday_agg_by_protocol.csv"
    os.makedirs(f"{options.train_data_folder}Agg\\AggregateByProtocol\\", exist_ok=True)

    #files.extend(glob.glob(os.path.join(options.train_data_folder, "2_Tuesday*.csv")))
    #output_path = f"{options.train_data_folder}Agg\\AggregateByProtocol\\Tuesday_agg_by_protocol.csv"
    #os.makedirs(f"{options.train_data_folder}Agg\\AggregateByProtocol\\", exist_ok=True)

    #files.extend(glob.glob(os.path.join(options.train_data_folder, "3_Wednesday*.csv")))
    #output_path = f"{options.train_data_folder}Agg\\AggregateByProtocol\\Wednesday_agg_by_protocol.csv"
    #os.makedirs(f"{options.train_data_folder}Agg\\AggregateByProtocol\\", exist_ok=True)

    #files.extend(glob.glob(os.path.join(options.train_data_folder, "4_Thursday*.csv")))
    #output_path = f"{options.train_data_folder}Agg\\AggregateByProtocol\\Thursday_agg_by_protocol.csv"
    #os.makedirs(f"{options.train_data_folder}Agg\\AggregateByProtocol\\", exist_ok=True)

    #files.extend(glob.glob(os.path.join(options.train_data_folder, "5_Friday*.csv")))
    #output_path = f"{options.train_data_folder}Agg\\AggregateByProtocol\\Friday_agg_by_protocol.csv"
    #os.makedirs(f"{options.train_data_folder}Agg\\AggregateByProtocol\\", exist_ok=True)

    processor = AggregationByProtocol()
